...
import { SystemSettingsSchema } from '@/models/schemas/system-settings';
...
export async function GET() {
  try {
    ...
    await dbConnect();
    const data = await SystemSettings.find().sort('-effectiveDate').lean();
...
/** POST：新增一筆系統參數版本（每一筆即一個版本，不覆蓋舊資料） */
export async function POST(req: NextRequest) {
  try {
    ...
    await dbConnect();
    const data = await SystemSettings.create(validated);
