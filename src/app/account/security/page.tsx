...
const SECURITY_FIELD_KEYS: Array<keyof SecurityFormValues> = ['currentPassword', 'newPassword', 'confirmPassword'];
        ...
        if (result?.errors && typeof result.errors === 'object') {
          const errorFields = Object.entries(result.errors)
            .filter(([name, messages]) => {
              return (
                SECURITY_FIELD_KEYS.includes(name as keyof SecurityFormValues) &&
                Array.isArray(messages) &&
                messages.length > 0
              );
            })
            .map(([name, messages]) => ({
              name: name as keyof SecurityFormValues,
  ...
  return (
    <section>
      ...
      <Card variant="borderless">
        <Form<SecurityFormValues> form={form} layout="vertical" onFinish={onSubmit}>
